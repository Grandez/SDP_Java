...
@Entity
@Table(name="SES_PARRAFOS")
@NamedQueries({
	@NamedQuery(name="SESParrafo.ParrafosBySesion"
			   ,query="SELECT p FROM SESParrafo p     " +
                      "         WHERE p.idSesion = ?1 " +
	                  "         ORDER BY p.idVersion, p.nombre "
			   )
   ,@NamedQuery(name="SESParrafo.ParrafosByName"
               ,query="SELECT p FROM SESParrafo p "      +
                      "         WHERE p.idSesion  = ?1 " +
                      "         AND   p.idVersion = ?2 " + 
...
public class SESParrafo implements Serializable {

    private static final long serialVersionUID = -4067477783827722568L;
   
    public static final String SumParrafos = 
            "SELECT idModulo, nombre, COUNT(*), MAX(orden) " +
            ...
            "FROM  TRP_PARRAFOS "            +        
            "WHERE idSesion = ?1 "           +
            "AND   orden BETWEEN ?2 AND ?3 " +
            "GROUP idModulo, nombre ";
   ...
   Long avgIntSuspend;
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        SESParrafo other = (SESParrafo) obj;
        if (avgIntCpu == null) {
