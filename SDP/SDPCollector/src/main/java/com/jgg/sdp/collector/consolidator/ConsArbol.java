...
public class ConsArbol {
    ...
    private MODDependenciaService depService = new MODDependenciaService();    
    ...
    private void updateDependencias(MODVersion verCalling, MODVersion verCalled, String nombre) {
        MODDependencia dep = depService.find(verCalling.getIdVersion(), nombre);
        // Si no existe solo es un modulo
        if (dep == null) {
            dep = new MODDependencia();
            dep.setIdVersion(verCalling.getIdVersion());
            dep.setTipo(CDG.DEP_MOD_DYNAMIC);
            dep.setModulo(nombre);
        }
        dep.setEstado(CDG.DEP_CHECKED);
