...
public class Args {
	...
	private void loadDefaults() {
		   String def[][] = { 
			           {"h", "h" , "help"   , "" , CFG.HELP       , "0" , "222" , STRING}
	                  ,{"" , ""  , "config" , "" , CFG.CONFIG     , "1" , "215" , FILE  }
	...
	private int processParm(String[] args, int idx, int pos) {
		...
		cfg.setParameter(prm[4], args[next]);
		return next;
	}
	...
	private void loadEnvironment() {
		...
		for (int idx = 0; idx < prms.size(); idx++) {
			...
			if (var != null) {
				cfg.setParameter(prms.get(idx)[4],var);
			}
		}
	}
