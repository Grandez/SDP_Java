...
public abstract class AbstractService <T>  {
	...
	public AbstractService() {
        ...
        em = DBManagerFactory.getEntityManager();
    }
...
//    @PersistenceContext
	...
	protected void updateQuery(String qryName, Object... keys) {
		Query query = em.createNamedQuery(tableName + "." + qryName);
        for (int idx = 0; idx < keys.length; idx++) {
            query.setParameter(idx+1, keys[idx]);    
        }
    ...
    protected T findQuery(Query query, Object... keys) {
    ...
    protected void nativeUpdate(String stmt, Object... keys) {
        Query qry = em.createNativeQuery(stmt);
        addParameters(qry, keys);
        qry.executeUpdate();
    }
